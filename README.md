...
Продвинутый трекер задач. Авторизация по токену через djoser. Задачи имеют приоритеты. Можно фильтровать и сортировать по приоритетам. На задачи можно назначать исполнителя. К задачам можно создавать подзадачи. Есть эндпоинты для получения различных статистических данных по задачам.

Сервис задеплоен на серевер Yandex Cloud, CI/CD настроен через GitHub Workflows, написаны базовые тесты, используются gunicorn, nginx, docker compose, PostgreSQL.
...
* На сервере уже немного заполнена база данных для тестовых целей, можно регистрироваться и обращаться по всем эндпоинтам.
...
```
* Так, как используется Djoser - все его эндпоинты для операций с пользователем также доступны (смотри redoc)

5) Создаие задачи:
* Отправить Post-запрос http://127.0.0.1:8000/api/tasks/.
```
{
    "title": "string",
    "description": "string",
...
```
* Опционально можно указать приоритет(low по умолчанию), существующую категорию и указать другого исполнителя(по умолчанию это автор задачи)

* Для получения списка всех задач отправить Get-запрос http://127.0.0.1:8000/api/tasks/.
...
* Ответ придёт в форме списка метрик и значений.
...
* Отправить пустой POST-запрос http://127.0.0.1:8000/api/tasks/<task_id>/complete_task/. (Нужно быть автором или исполнителем)
